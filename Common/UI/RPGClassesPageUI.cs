...
namespace Wolfgodrpg.Common.UI
{
    ...
    public class RPGClassesPageUI : UIElement
    {
        ...
        {
            public ClassEntry(string className, float level, float currentExp, float nextLevelExp, Dictionary<int, string> milestones, Dictionary<string, float> statBonuses, bool compact)
            {
                Width.Set(0, 1f);
                ...
                if (milestones != null && milestones.Count > 0)
                {
                    ...
                    var unlockedMilestones = milestones.Where(m => m.Key <= level).OrderBy(m => m.Key).ToList();
                    if (unlockedMilestones.Any())
                    {
                        foreach (var milestone in unlockedMilestones.TakeLast(compact ? 1 : 2))
                        {
                            ...
                            var unlockedText = new UIText($"  ✅ Nv.{milestone.Key}: {abilityText}", compact ? 0.55f : 0.6f);
                    ...
                    if (!string.IsNullOrEmpty(nextMilestone.Value))
                    {
                        ...
                        int levelsNeeded = nextMilestone.Key - (int)level;
                        
                        var nextText = new UIText($"  🔒 Nv.{nextMilestone.Key}: {abilityText} ({levelsNeeded} níveis)", compact ? 0.55f : 0.6f);
